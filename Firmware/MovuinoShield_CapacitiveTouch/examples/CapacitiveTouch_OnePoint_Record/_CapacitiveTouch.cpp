...
unsigned int CapacitiveTouch::getRawData(int rx, int tx){
    beginWireIfNull();
    // Start scan
    i2cWriteByte(I2C_ADDRESS,0x00,0xc0);
    unsigned int data = 0;
    ...
    delayMicroseconds(50); // Wait at least 50us
    uint8_t r = i2cRead(I2C_ADDRESS, byte(0x10), 2*NUM_RX, result);
    if (r>0 && DEBUG) Serial.println("PROBLEM");

    data = (result[2 * rxAddr] << 8) | (result[2 * rxAddr + 1]);
